...
$view_user_id = intval($_GET["userid"]);

if($user_id === $view_user_id) {
    // user tries to view his profile, redirect him to index.php
    header("Location: index.php");
    exit();
}
...
while($stmt_f->fetch()) {
    ...
    if(!$is_following) {
        if($follower_id === $user_id && $following_id = $view_user_id) {
